...
bookingForm.addEventListener('submit', function(e) {
    ...
    const now = new Date('2025-06-17T21:34:00+07:00'); // Thời gian hiện tại
...
const nowPicker = new Date('2025-06-17T21:34:00+07:00');
...
document.addEventListener('DOMContentLoaded', function() {
    async function fetchStaffAvailability() {
        ...
        try {
            ...
            const url = `${bookingForm.action}?date=${encodeURIComponent(formattedDate)}&startTime=${encodeURIComponent(startTime)}&duration=${duration}`;
            ...
            const response = await fetch(url, {
                ...
                mode: 'cors' // Thêm mode cors để xử lý preflight
            });
            ...
            staffLoading.style.display = 'block';
            staffList.innerHTML = '';
            staffLoading.innerHTML = `<p style="color:red;">Could not load staff list. Please try again. Error: ${error.message}</p>`;
            // Thêm retry logic
            if (error.message.includes('404') || error.message.includes('CORS')) {
                console.log('Attempting to retry after 2 seconds...');
                setTimeout(fetchStaffAvailability, 2000); // Retry after 2 seconds
            }
        }
    }
    ...
    function displayStaff(staffList) {
        ...
        if (Array.isArray(staffList)) {
            ...
            staffList.innerHTML = '<li>No staff data available.</li>';
        }
    }
